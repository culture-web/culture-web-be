...
const deriveUsername = (user) => {
  const fromMetadata =
    ...
    || user?.email?.split('@')?.[0];
  return String(fromMetadata || `user-${String(user?.id || '').slice(0, 8)}`).trim();
};
...
const mapSupabaseUserToManagedUser = (user) => ({
  ...
  updated_at: toIsoString(user.updated_at || user.last_sign_in_at || user.created_at),
});
...
const createKbUserInSupabase = async ({ username, email, password, role }) => {
  ...
  if (error) {
    ...
    if (/already\s+registered|already\s+exists|already\s+been\s+registered/i.test(message)) {
